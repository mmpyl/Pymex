/**
 * Inventario Presentation Components
 * Componentes React para la visualización del Inventario
 */

import { useEffect, useState } from 'react';
import toast from 'react-hot-toast';
import { inventarioService } from '../application/service.js';
import { MovimientoInventario } from '../domain/entities.js';
...
export default function Inventario() {
  ...
  const cargar = async () => {
    try {
      setLoading(true);
      const data = await inventarioService.loadInventarioData();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const result = await inventarioService.registrarMovimiento(form);
      toast.success(`Movimiento registrado. Stock actual: ${result.stock_actual}`);
  ...
  const estadisticas = inventarioService.getEstadisticas(productos, historial);
    ...
    <div className="flex-1 space-y-5 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {estadisticas.map((k) => (
          <div
            key={k.label}
            className={`flex items-center gap-3 rounded-xl border-l-4 bg-white p-4 shadow-sm ${k.border}`}
          >
      ...
      {mostrarForm && (
        <div className="rounded-xl bg-white p-5 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <button
              className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white xl:col-start-5"
              type="submit"
            >
              Registrar
            </button>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ['stock', 'Stock actual'],
          ['historial', 'Historial'],
          ['stock-bajo', `Stock bajo (${productosBajoStock.length})`],
        ].map(([key, label]) => (
          <button
            key={key}
            onClick={() => setVista(key)}
            className={`rounded-md px-4 py-2 text-sm font-semibold ${
              vista === key ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-600'
            }`}
          >
            {label}
          </button>
          ...
          <tbody>
            {productos.map((p) => (
              <tr key={p.id} className="border-t border-slate-100">
                <Td>{p.nombre}</Td>
                <Td>
                  <span className={p.estaBajoStock ? 'font-bold text-red-600' : 'font-bold text-emerald-600'}>
                    {p.stock}
                  </span>
                </Td>
                ...
                <Td>
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                      p.estaBajoStock ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'
                    }`}
                  >
                    {p.estaBajoStock ? '⚠️ Bajo' : '✅ OK'}
                  </span>
                </Td>
              </tr>
            ))}
      ...
      {vista === 'historial' && (
        <TableShell>
          ...
          <tbody>
            {historial.map((m) => {
              const config = MovimientoInventario.getTipoConfig(m.tipo);
              return (
              ...
              <tbody>
                {productosBajoStock.map((p) => (
                  <tr key={p.id} className="border-t border-slate-100">
                    <Td>{p.nombre}</Td>
                    <Td>
                      <strong className="text-red-600">{p.stock}</strong>
                    </Td>
                    <Td>{p.stock_minimo}</Td>
                    <Td>
                      <span className="font-bold text-red-600">-{p.diferenciaStock}</span>
                    </Td>
                  </tr>
                ))}
              </tbody>
