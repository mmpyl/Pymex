...
class FacturaController
{
    ...
    public function emitir(Request $req, Response $res): Response
    {
        ...
        try {
            ...
            $serie = $body['serie'] ?? 'F001';

            if (empty($body['cliente']['ruc'])) return $this->error($res, 'El RUC del cliente es obligatorio para facturas', 400);
            if ($empresaId <= 0) return $this->error($res, 'empresa_id inválido', 400);
            ...
            $datos = [
                ...
                'empresa' => [
                    'ruc' => $empresa['ruc'] ?? ($_ENV['SUNAT_RUC'] ?? ''),
            ...
            $resultado = $this->greenter->generarFactura($datos);
            ...
            return $this->json($res, [
                ...
                'numero' => $numero,
                'estado' => $estado,
                'sunat_codigo' => $resultado['codigo'],
                'descripcion' => $resultado['descripcion'],
                'hash' => $resultado['hash'],
                'pdf_url' => $pdfPath ? "/api/facturas/{$comprobanteId}/pdf" : null,
                'xml_url' => "/api/facturas/{$comprobanteId}/xml",
            ]);
        } catch (\Exception $e) {
    ...
    {
        $comprobante = $this->sunat->obtenerComprobante((int)$args['id']);
        return $comprobante ? $this->json($res, $comprobante) : $this->error($res, 'Comprobante no encontrado', 404);
    }

    public function descargarPdf(Request $req, Response $res, array $args): Response
    {
        $comprobante = $this->sunat->obtenerComprobante((int)$args['id']);
        if (!$comprobante || !$comprobante['pdf_path']) return $this->error($res, 'PDF no disponible', 404);

        $res->getBody()->write(file_get_contents($comprobante['pdf_path']));
        return $res->withHeader('Content-Type', 'application/pdf')
            ->withHeader('Content-Disposition', "attachment; filename={$comprobante['numero']}.pdf");
    }

    public function descargarXml(Request $req, Response $res, array $args): Response
    {
        $comprobante = $this->sunat->obtenerComprobante((int)$args['id']);
        if (!$comprobante || !$comprobante['xml_path']) return $this->error($res, 'XML no disponible', 404);

        $res->getBody()->write(file_get_contents($comprobante['xml_path']));
        return $res->withHeader('Content-Type', 'application/xml')
            ->withHeader('Content-Disposition', "attachment; filename={$comprobante['numero']}.xml");
    }

    public function reenviar(Request $req, Response $res, array $args): Response
    {
        return $this->json($res, ['mensaje' => 'Reenvio en proceso', 'id' => (int)$args['id']]);
    }

    public function listarPorEmpresa(Request $req, Response $res, array $args): Response
    {
        return $this->json($res, $this->sunat->listarPorEmpresa((int)$args['empresa_id']));
    }

    public function getSiguienteCorrelativo(Request $req, Response $res, array $args): Response
    {
        return $this->json($res, ['correlativo' => $this->sunat->getSiguienteCorrelativo((int)$args['empresa_id'], $args['serie'])]);
    }
