...
// FIX: la función audit() ahora usa req.admin?.id (token admin) con fallback
// a req.usuario?.id (compatibilidad), porque las rutas /api/admin/* ahora
// usan verificarTokenAdmin que puebla req.admin, no req.usuario.

const { Op, fn, col, literal } = require('sequelize');
const {
  sequelize, Empresa, Usuario, Plan, Feature,
  PlanFeature, PlanLimit, Suscripcion, FeatureOverride,
  Pago, AuditoriaAdmin, Rubro, EmpresaRubro
...
const audit = async (req, accion, entidad, entidad_id, detalles = {}) => {
  await AuditoriaAdmin.create({
    admin_usuario_id: req.admin?.id || req.usuario?.id,
    accion,
    entidad,
    entidad_id,
    detalles,
...
// FIX: usar req.admin?.id (rutas admin) con fallback a req.usuario?.id
const audit = async (req, accion, entidad, entidad_id, detalles = {}) => {
  try {
    ...
    await AuditoriaAdmin.create({
      ...
      accion, entidad, entidad_id, detalles, ip: req.ip
    });
  } catch {
    // Auditoría nunca bloquea la operación principal
  }
};
  ...
  try {
    ...
    const [
      ...
      Pago.findAll({
        ...
        where: {
          estado: 'pagado',
          creado_en: { [Op.gte]: inicioMesAnterior, [Op.lt]: inicioMesActual }
        }
      }),
      Suscripcion.findAll({
    ...
    const mrrActual = Number(mrrActualRows[0]?.dataValues?.mrr || 0);
      ...
      Pago.findAll({ attributes: [[fn('COALESCE', fn('SUM', col('monto')), 0), 'mrr']], where: { estado: 'pagado', creado_en: { [Op.gte]: inicioMesActual } } }),
      Pago.findAll({ attributes: [[fn('COALESCE', fn('SUM', col('monto')), 0), 'mrr']], where: { estado: 'pagado', creado_en: { [Op.gte]: inicioMesAnterior, [Op.lt]: inicioMesActual } } }),
      Suscripcion.findAll({ attributes: ['plan_id', [fn('COUNT', col('plan_id')), 'cantidad']], include: [{ model: Plan, attributes: ['nombre', 'codigo'] }], group: ['plan_id', 'Plan.id'], order: [[literal('cantidad'), 'DESC']], limit: 5 }),
      Pago.count({ where: { estado: { [Op.in]: ['pendiente', 'vencido'] } } })
    ]);
...
const listarUsuariosEmpresa = async (req, res) => {
  const users = await Usuario.findAll({ where: { empresa_id: req.params.id }, order: [['id', 'DESC']] });
...
const listarPlanes = async (_req, res) => {
  const planes = await Plan.findAll({ include: [PlanFeature, PlanLimit], order: [['precio_mensual', 'ASC']] });
...
const listarFeatures = async (_req, res) => res.json(await Feature.findAll({ order: [['id', 'DESC']] }));
...
const asignarRubrosEmpresa = async (req, res) => {
  ...
  const actualizados = await Empresa.findByPk(empresa.id, { include: [{ model: Rubro, through: { attributes: [] } }] });
  await audit(req, 'asignar_rubros_empresa', 'empresa', empresa.id, { rubro_ids: rubroIds });

  return res.json({ empresa_id: empresa.id, rubros: actualizados?.Rubros || [] });
};
...
const listarSuscripciones = async (_req, res) => {
  const rows = await Suscripcion.findAll({ include: [Empresa, Plan], order: [['id', 'DESC']] });
  return res.json(rows);
};

const crearSuscripcion = async (req, res) => {
  const missing = requireFields(req.body, ['empresa_id', 'plan_id']);
  if (missing) return res.status(400).json({ error: missing });
  ...
  try {
    await Suscripcion.update(
      { estado: 'cancelada', fecha_fin: new Date() },
    ...
    if (plan) {
      await Empresa.update({ plan: plan.codigo }, { where: { id: req.body.empresa_id }, transaction: tx });
    }
...
const registrarPago = async (req, res) => {
  ...
  if (!ESTADOS_PAGO.includes(estado)) return res.status(400).json({ error: 'Estado de pago inválido' });
...
const generarCheckoutPago = async (req, res) => {
  ...
  await audit(req, 'generar_checkout_pago', 'pago', pago.id, { checkout_url: checkout.checkout_url, provider: checkout.provider });

  return res.json({ pago_id: pago.id, checkout_url: checkout.checkout_url, provider: checkout.provider });
};
...
const listarPagos = async (req, res) => {
  ...
  return res.json(pagos);
      ...
      churn_pct: empresasActivas > 0 ? Number(((cancelaciones / empresasActivas) * 100).toFixed(2)) : 0,
      planes_mas_usados: planesMasUsados, crecimiento_mensual_pct: crecimientoMensualPct
    });
  } catch (error) { return err500(res, error); }
};
...
const listarEmpresas = async (req, res) => {
  try {
    ...
    const { rows, count } = await Empresa.findAndCountAll({
      where,
      include: [{ model: Rubro, through: { attributes: [] } }],
      order: [['id', 'DESC']], limit: pageSize, offset
    });
    return res.json({ data: rows, meta: { page, pageSize, total: count } });
  } catch (error) { return err500(res, error); }
};
...
const crearEmpresa = async (req, res) => {
  try {
    ...
    if (missing) return res.status(400).json({ error: missing });
    const estado = req.body.estado || 'activo';
    if (!ESTADOS_EMPRESA.includes(estado)) return res.status(400).json({ error: 'Estado de empresa inválido' });
    const empresa = await Empresa.create({ nombre: req.body.nombre, email: req.body.email, ruc: req.body.ruc || null, plan: req.body.plan || 'basico', estado });
    await audit(req, 'crear_empresa', 'empresa', empresa.id, req.body);
    return res.status(201).json(empresa);
...
const actualizarEmpresa = async (req, res) => {
  try {
    const empresa = await Empresa.findByPk(req.params.id);
    if (!empresa) return res.status(404).json({ error: 'Empresa no encontrada' });
    if (req.body.estado && !ESTADOS_EMPRESA.includes(req.body.estado)) return res.status(400).json({ error: 'Estado de empresa inválido' });
    await empresa.update(req.body);
    await audit(req, 'actualizar_empresa', 'empresa', empresa.id, req.body);
    return res.json(empresa);
  } catch (error) { return err500(res, error); }
...
const cambiarEstadoEmpresa = async (req, res) => {
  try {
    const empresa = await Empresa.findByPk(req.params.id);
    if (!empresa) return res.status(404).json({ error: 'Empresa no encontrada' });
    if (!ESTADOS_EMPRESA.includes(req.body.estado)) return res.status(400).json({ error: `Estado inválido. Usa: ${ESTADOS_EMPRESA.join(', ')}` });
    await empresa.update({ estado: req.body.estado });
    await audit(req, 'cambiar_estado_empresa', 'empresa', empresa.id, { estado: req.body.estado });
    return res.json({ mensaje: 'Estado actualizado', empresa });
  } catch (error) { return err500(res, error); }
...
const actualizarUsuario = async (req, res) => {
  try {
    const user = await Usuario.findByPk(req.params.usuarioId);
    if (!user || String(user.empresa_id) !== String(req.params.id)) return res.status(404).json({ error: 'Usuario no encontrado para la empresa indicada' });
    const patch = {};
...
// ─── RUBROS ───────────────────────────────────────────────────────────────────
const listarRubros = async (_req, res) => {
  try { return res.json(await Rubro.findAll({ order: [['nombre', 'ASC']] })); }
  catch (error) { return err500(res, error); }
};
...
const crearRubro = async (req, res) => {
  try {
    const missing = requireFields(req.body, ['nombre']);
    if (missing) return res.status(400).json({ error: missing });
    const rubro = await Rubro.create({ nombre: req.body.nombre.trim(), descripcion: req.body.descripcion || null });
    await audit(req, 'crear_rubro', 'rubro', rubro.id, req.body);
    return res.status(201).json(rubro);
  } catch (error) { return res.status(400).json({ error: error.message }); }
};
...
const actualizarRubro = async (req, res) => {
  try {
    const rubro = await Rubro.findByPk(req.params.id);
    if (!rubro) return res.status(404).json({ error: 'Rubro no encontrado' });
    await rubro.update({ nombre: req.body.nombre ?? rubro.nombre, descripcion: req.body.descripcion ?? rubro.descripcion });
    await audit(req, 'actualizar_rubro', 'rubro', rubro.id, req.body);
    return res.json(rubro);
  } catch (error) { return err500(res, error); }
};
...
const asignarRubrosEmpresa = async (req, res) => {
  try {
    ...
    if (!empresa) return res.status(404).json({ error: 'Empresa no encontrada' });
    const rubroIds = Array.isArray(req.body.rubro_ids) ? req.body.rubro_ids.map(Number) : [];
    if (!rubroIds.length) return res.status(400).json({ error: 'rubro_ids debe incluir al menos un id' });
    const rubros = await Rubro.findAll({ where: { id: { [Op.in]: rubroIds } } });
    if (rubros.length !== rubroIds.length) return res.status(400).json({ error: 'Uno o más rubros no existen' });
    await EmpresaRubro.destroy({ where: { empresa_id: empresa.id } });
    await EmpresaRubro.bulkCreate(rubroIds.map((rubroId) => ({ empresa_id: empresa.id, rubro_id: rubroId })));
    const actualizada = await Empresa.findByPk(empresa.id, { include: [{ model: Rubro, through: { attributes: [] } }] });
    await audit(req, 'asignar_rubros_empresa', 'empresa', empresa.id, { rubro_ids: rubroIds });
...
// ─── PLANES ───────────────────────────────────────────────────────────────────
const listarPlanes = async (_req, res) => {
  try { return res.json(await Plan.findAll({ include: [PlanFeature, PlanLimit], order: [['precio_mensual', 'ASC']] })); }
  catch (error) { return err500(res, error); }
};
...
const crearPlan = async (req, res) => {
  try {
    const missing = requireFields(req.body, ['nombre', 'codigo']);
    if (missing) return res.status(400).json({ error: missing });
    return res.status(201).json(await Plan.create(req.body));
  } catch (error) { return res.status(400).json({ error: error.message }); }
};
...
const listarFeatures = async (_req, res) => {
  try { return res.json(await Feature.findAll({ order: [['id', 'DESC']] })); }
  catch (error) { return err500(res, error); }
};
...
const crearFeature = async (req, res) => {
  try {
    const missing = requireFields(req.body, ['nombre', 'codigo']);
    if (missing) return res.status(400).json({ error: missing });
    return res.status(201).json(await Feature.create(req.body));
  } catch (error) { return res.status(400).json({ error: error.message }); }
};
...
const guardarLimitePlan = async (req, res) => {
  try {
    if (!req.body.limite || req.body.valor === undefined) return res.status(400).json({ error: 'limite y valor son requeridos' });
    const [limit, created] = await PlanLimit.findOrCreate({
      where: { plan_id: req.params.id, limite: req.body.limite }, defaults: { valor: req.body.valor }
    });
    if (!created) await limit.update({ valor: req.body.valor });
    return res.json(limit);
  } catch (error) { return err500(res, error); }
...
const crearOverride = async (req, res) => {
  try {
    if (!req.body.feature_id || typeof req.body.activo !== 'boolean') return res.status(400).json({ error: 'feature_id y activo (boolean) son requeridos' });
    const [override, created] = await FeatureOverride.findOrCreate({
      where: { empresa_id: req.params.id, feature_id: req.body.feature_id },
      defaults: { activo: req.body.activo, motivo: req.body.motivo }
    });
    if (!created) await override.update({ activo: req.body.activo, motivo: req.body.motivo || override.motivo });
    await audit(req, 'override_feature_empresa', 'feature_override', override.id, req.body);
    clearFeatureCache(req.params.id);
    return res.status(created ? 201 : 200).json(override);
  } catch (error) { return err500(res, error); }
...
// ─── SUSCRIPCIONES ────────────────────────────────────────────────────────────
const listarSuscripciones = async (_req, res) => {
  try { return res.json(await Suscripcion.findAll({ include: [Empresa, Plan], order: [['id', 'DESC']] })); }
  catch (error) { return err500(res, error); }
};
...
const crearSuscripcion = async (req, res) => {
  try {
    ...
    if (missing) return res.status(400).json({ error: missing });
    const estado = req.body.estado || 'activa';
    if (!ESTADOS_SUSCRIPCION.includes(estado)) return res.status(400).json({ error: 'Estado de suscripción inválido' });
    const tx = await sequelize.transaction();
    try {
      await Suscripcion.update({ estado: 'cancelada', fecha_fin: new Date() }, { where: { empresa_id: req.body.empresa_id, estado: 'activa' }, transaction: tx });
      const suscripcion = await Suscripcion.create({ empresa_id: req.body.empresa_id, plan_id: req.body.plan_id, estado, fecha_inicio: req.body.fecha_inicio || new Date(), fecha_fin: req.body.fecha_fin || null, auto_renovacion: req.body.auto_renovacion !== false }, { transaction: tx });
      const plan = await Plan.findByPk(req.body.plan_id, { transaction: tx });
      if (plan) await Empresa.update({ plan: plan.codigo }, { where: { id: req.body.empresa_id }, transaction: tx });
      await tx.commit();
      await audit(req, 'crear_suscripcion', 'suscripcion', suscripcion.id, req.body);
      clearFeatureCache(req.body.empresa_id);
      return res.status(201).json(suscripcion);
    } catch (inner) { await tx.rollback(); return res.status(400).json({ error: inner.message }); }
  } catch (error) { return err500(res, error); }
};
...
const cambiarPlanSuscripcion = async (req, res) => {
  try {
    const suscripcion = await Suscripcion.findByPk(req.params.id);
    if (!suscripcion) return res.status(404).json({ error: 'Suscripción no encontrada' });
    if (req.body.estado && !ESTADOS_SUSCRIPCION.includes(req.body.estado)) return res.status(400).json({ error: 'Estado de suscripción inválido' });
    await suscripcion.update(req.body);
    await audit(req, 'actualizar_suscripcion', 'suscripcion', suscripcion.id, req.body);
    clearFeatureCache(suscripcion.empresa_id);
    return res.json(suscripcion);
...
const listarPagos = async (req, res) => {
  try {
    const where = {};
    if (req.query.estado) where.estado = req.query.estado;
    return res.json(await Pago.findAll({ include: [Empresa, Suscripcion], where, order: [['id', 'DESC']] }));
  } catch (error) { return err500(res, error); }
};
...
const marcarPagoPagado = async (req, res) => {
  try {
    const pago = await applyPaymentAndReactivate(req.params.id, req.body?.referencia || null);
    await audit(req, 'marcar_pago_pagado', 'pago', pago.id, { referencia: req.body?.referencia });
    return res.json({ ok: true, pago_id: pago.id, estado: 'pagado' });
  } catch (error) { return err500(res, error); }
};
...
const listarAuditoria = async (req, res) => {
  try {
    const { page, pageSize, offset } = parsePagination(req);
    const { rows, count } = await AuditoriaAdmin.findAndCountAll({
      order: [['creado_en', 'DESC']], limit: pageSize, offset
    });
    return res.json({ data: rows, meta: { page, pageSize, total: count } });
  } catch (error) { return err500(res, error); }
};
...
const metricasSaas = async (_req, res) => {
  try {
    ...
    for (let i = 5; i >= 0; i -= 1) {
      ...
      // eslint-disable-next-line no-await-in-loop
      const [mrrRows, nuevas, canceladas] = await Promise.all([
        Pago.findAll({ attributes: [[fn('COALESCE', fn('SUM', col('monto')), 0), 'mrr']], where: { estado: 'pagado', creado_en: { [Op.gte]: inicio, [Op.lt]: fin } } }),
        Suscripcion.count({ where: { creado_en: { [Op.gte]: inicio, [Op.lt]: fin } } }),
        Suscripcion.count({ where: { estado: 'cancelada', actualizado_en: { [Op.gte]: inicio, [Op.lt]: fin } } })
      ]);
      meses.push({ mes: inicio.toISOString().slice(0, 7), mrr: Number(mrrRows[0]?.dataValues?.mrr || 0), nuevas_suscripciones: nuevas, cancelaciones: canceladas });
    }
    return res.json({ series_6m: meses });
  } catch (error) { return err500(res, error); }
...
module.exports = {
  ...
  listarEmpresas, crearEmpresa, actualizarEmpresa, cambiarEstadoEmpresa, eliminarEmpresa, detalleEmpresa,
  listarUsuariosEmpresa, actualizarUsuario,
  listarRubros, crearRubro, actualizarRubro, asignarRubrosEmpresa,
  listarPlanes, crearPlan, actualizarPlan,
  listarFeatures, crearFeature, asignarFeaturePlan, guardarLimitePlan,
  crearOverride, featuresEfectivosEmpresa,
  ejecutarCobranza,
  listarSuscripciones, crearSuscripcion, cambiarPlanSuscripcion,
  registrarPago, generarCheckoutPago, marcarPagoPagado, listarPagos,
  listarAuditoria, metricasSaas
