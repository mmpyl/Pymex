...
router.put('/empresas/:id', async (req, res) => {
  try {
    ...
    allowed.forEach((key) => {
      if (req.body[key] !== undefined) empresa[key] = req.body[key];
    });
    await empresa.save();
    res.json(empresa);
  } catch (error) {
...
router.patch('/empresas/:id/estado', async (req, res) => {
  try {
    ...
    empresa.estado = req.body.estado === 'activo' ? 'activo' : 'inactivo';
    await empresa.save();
    res.json({ id: empresa.id, estado: empresa.estado });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/planes', async (req, res) => {
  try {
    const { nombre, codigo, descripcion = '', precio = 0, max_usuarios = 3, max_productos = 200, max_ventas_mes = 500, estado = true } = req.body;
    const plan = await Plan.create({ nombre, codigo, descripcion, precio, max_usuarios, max_productos, max_ventas_mes, estado });
...
router.put('/planes/:id', async (req, res) => {
  try {
    ...
    const fields = ['nombre', 'codigo', 'descripcion', 'precio', 'max_usuarios', 'max_productos', 'max_ventas_mes', 'estado'];
    fields.forEach((key) => {
...
router.get('/suscripciones', async (req, res) => {
  try {
    const list = await Suscripcion.findAll({ include: [{ model: Empresa, attributes: ['id', 'nombre', 'estado'] }], order: [['id', 'DESC']] });
    res.json(list);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.patch('/suscripciones/:id/plan', async (req, res) => {
  try {
    ...
    suscripcion.plan = req.body.plan;
    if (req.body.monto_mensual !== undefined) suscripcion.monto_mensual = req.body.monto_mensual;
    await suscripcion.save();

    await Empresa.update({ plan: req.body.plan }, { where: { id: suscripcion.empresa_id } });
...
router.patch('/suscripciones/:id/suspender', async (req, res) => {
  try {
    ...
    res.json({ ok: true, suscripcion_id: suscripcion.id, estado: 'suspendida' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/pagos', async (req, res) => {
  try {
    const payload = {
      ...
      suscripcion_id: req.body.suscripcion_id || null,
    ...
    const pago = await Pago.create(payload);
...
router.get('/pagos', async (req, res) => {
  try {
    ...
    const pagos = await Pago.findAll({ include: [{ model: Empresa, attributes: ['id', 'nombre'] }], order: [['id', 'DESC']] });
    res.json(pagos);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.post('/admins', async (req, res) => {
  try {
    ...
    const user = await Usuario.create({ empresa_id, rol_id, nombre, email, password: passwordHash, estado: 'activo' });

    res.status(201).json({ id: user.id, empresa_id: user.empresa_id, rol_id: user.rol_id, nombre: user.nombre, email: user.email });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const [totalEmpresas, activas, mrr, nuevasEmpresas, planes, totalSubs, canceladas] = await Promise.all([
      Empresa.count(),
      ...
      Suscripcion.findAll({
        attributes: ['plan', [fn('COUNT', col('Suscripcion.id')), 'total']],
        group: ['plan'],
    ...
    res.json({
      ...
      mrr: Number(mrr || 0),
      churn,
      plan_mas_usado: planes[0]?.plan || null,
