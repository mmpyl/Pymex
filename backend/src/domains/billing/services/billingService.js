// backend/src/domains/billing/services/billingService.js — versión corregida y migrada a dominios
...
const billingModels = require('../models');
  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      try {
        ...
        if (affectedEmpresas.length) {
          try {
            const { emailPagoVencido } = require('../../shared-kernel/services/emailService');
